...
+(id)managerForDCARS{
    return [[self alloc]initWithDBPath:[self documentsPath:@"DCARS.sqlite"]];
}
-(id)init
{
...
- (BOOL)alterAutoTable:(NSString *)table newColumn:(NSString *)name type:(NSString *)type {
    NSString *cmd = [NSString stringWithFormat:@"ALTER TABLE %@ ADD COLUMN %@ %@", table, name, type];
    return  [self executeUpdate:cmd];
}

- (BOOL)createAutoTable:(NSString *)table fields:(NSArray *)fields
{
    if([fields count] < 1) {
    ...
    NSString *cmd = [NSString stringWithFormat:@"CREATE TABLE IF NOT EXISTS '%@'('%@' INTEGER PRIMARY KEY AUTOINCREMENT", table, [fields objectAtIndex:0]];
